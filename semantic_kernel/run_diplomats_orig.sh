...
then
    ...
    python src/train_vectors.py \
        -i '../data_semantic/text_diplomat_orig_version2' \
        -vo 'mdl/vectors_all_orig.pcl' \
        -mo 'mdl/w2v_all_orig.pcl'
else
...
do 
    ...
    do
        ...
        python src/build_kernel_new.py --model mdl/vectors_all_orig.pcl --seed $seedls --association $associations --norm True
        
        for prunings in $PRUNINGS
        do
            echo "[INFO] Processing $prunings file..."
            python src/build_graph_new.py --seed $seedls --type 'diplomat_orig' --association $associations --pruning $prunings
            echo "-----"
